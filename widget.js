...
cprequire_test(["inline:com-chilipeppr-widget-serialport"], function (sp) {
    if (debug) console.log("test running of " + sp.id);
      ...
      function() {
        ...
        cprequire(
          ...
          function(myObjWidgetSpconsole) {
            ...
            if (debug) console.log("Widget / Serial Port Console v1.7 just got loaded.", myObjWidgetSpconsole);
...
`,
        ...
        init: function (host, buffertype, defaultBaud, buffertypeDescription) {
            ...
            if (this.isInitted) {
            	if (debug) console.warn("you just initted the serial port json server widget again? huh? returning");
              return;
            }
            this.isInitted = true;

            if (debug) console.group("init of serial port widget");
           ...
           //   if ('bufferEncouragementMsg' in opt) buffertypeDescription = opt.bufferEncouragementMsg;
            ...
            if (buffertype != null && buffertype.length > 0) {
                ...
                if (debug) console.log("we have a buffertype being requested. buffertype:", this.buffertype);
            ...
            if (defaultBaud) {
                if (debug) console.log("setting defaultBaud:", defaultBaud);
            ...
            chilipeppr.subscribe("/" + this.id + "/dedupeOn", this, function (msg) {
                if (debug) console.log("spjs widget now in dedupe mode");
                that.isInDeDupeMode = true;
            });
            chilipeppr.subscribe("/" + this.id + "/dedupeOff", this, function (msg) {
                if (debug) console.log("spjs widget no longer in dedupe mode");
            ...
            if (debug) console.log("setting up onPlannerResumeSetup");
            ...
            if (debug) console.log(this.name + " done loading.");
            console.groupEnd();
        },
        setupSubnetScan: function() {
            ...
            pc.onicecandidate = function(ice){  //listen for candidate events
                ...
                if (debug) console.log('my IP: ', myIP);
                var subnet = myIP.replace(/\d+$/, "*");
                if (debug) console.log("my IP subnet:", subnet);
        ...
        showRecentServerList: function() {
          ...
          var that = this;
          if (debug) console.log("showRecentServerList:", serverList)
          if (serverList && Object.keys(serverList).length > 0) {
            ...
            for (var key in serverList) {
              ...
              elWs.click(key, function(evt) {
                if (debug) console.log("got click. evt:", evt, "data:", evt.data);
                that.disconnect();
        ...
        onVersion: function(version) {
            ...
            if (debug) console.log("got version cmd. version:", version);
        ...
        onSpjsName: function(spjsName) {
            if (debug) console.error("spjsName: " + spjsName);
        ...
        onCloudServerClick: function(evt) {
            if (debug) console.log("got onCloudServerClick. evt:", evt, "data:", evt.data);
            var wsUrl = $(evt.currentTarget).data("ws");
            if (debug) console.log("url from data attrib:", wsUrl);
            this.disconnect();
        ...
        bufferEncouragement: function(list, arg2) {
            if (debug) console.log("got bufferEncouragement port list:", list, "arg2:", arg2);
            //$('.com-chilipeppr-widget-serialport-bufferindicator').addClass('hidden');
        ...
        publishSingleSelectPort: function() {
            if (debug) console.log("got publishSingleSelectPort. isSingleSelectMode:", this.isSingleSelectMode, "singleSelectPort:", this.singleSelectPort, "portlist:", this.portlist);
            var port = null;
            var that = this;
            if (this.portlist && this.portlist != null) {
                this.portlist.forEach(function(item) {
                    ...
                    if (item.Name == that.singleSelectPort) {
                        if (debug) console.log("found it. item.Name:", item.Name);
                        port = item;
                    }
                });
            }
        ...
        onRemoteHostConnect: function() {
            if (debug) console.log("onRemoteHostConnect");
        ...
        setSingleSelectMode: function() {
            ...
            if (debug) console.log("setSingleSelectMode. port:", this.singleSelectPort);
        },
        getPortListCount: 0,
        ...
        disconnect: function() {
            ...
            if (debug) console.log("closing websocket:", this.conn);
            if (this.conn) {
                this.conn.close();
        ...
        onHistoryMenuClick: function(evt) {
            if (debug) console.log("got onHistoryMenuClick. data:", evt.data);
        ...
        consoleSetup: function () {
            ...
            $("#com-chilipeppr-widget-serialport-consoleform").submit(function (evt) {
                ...
                if (debug) console.log("got submit on form");
            ...
            $("#com-chilipeppr-widget-serialport-consoleform").keydown(function(evt) {
                ...
                if (evt.which == 38) {
                    ...
                    if (that.historyLastShownIndex < 0) {
                        if (debug) console.log("out of history to show. up arrow.");
                    ...
                    if (that.historyLastShownIndex >= that.history.length) {
                        if (debug) console.log("out of history to show. down arrow.");
                        that.historyLastShownIndex = that.history.length;
        ...
        appendLogOld: function (msg) {
            ...
            if (log.text().length > 5000) {
                // truncating log
                if (debug) console.log("Truncating log.");
        ...
        appendLog: function (msg) {
            ...
            if (this.logEls.log == null) {
                if (debug) console.log("lazy loading logEls. logEls:", this.logEls);
        ...
        sendBuffered: function(msg) {
            ...
            if (debug) console.log("inside sendBuffered. msg:", msg);
        ...
        sendBufferedDoNext: function() {
            ...
            if (this.sendbuf.length == 0) {
                if (debug) console.log("no more items on buffer so exiting and not queuing for next");
                this.appendLog("(No more items on buffer.)");
                return;
            }
            
            if (this.isSendBufWaiting) {
                if (debug) console.log("isSendBufWaiting is true, so returning.");
                return;
            }

            var msg = this.sendbuf.join("");
            this.sendbuf = []; // clear buffer to empty array
            if (debug) console.log("sendBufferedDoNext. full join of buf. msg:", msg);
            /*
            ...
            */
            ...
            if (this.isSingleSelectMode) {
                if (this.singleSelectPort != null && this.singleSelectPort.length > 1) {
                    ...
                    if (debug) console.log("setting isSendBufWaiting to true. sending to port:", this.singleSelectPort, "msg:", msg);
        ...
        sendBufferedOnWsRecv: function(data) {
            if (debug) console.log("got sendBufferedOnWsRecv. setting isSendBufWaiting to false. data:", data);
        ...
        sendBufferedJson: function(msg) {
            ...
            // in this method we'll queue to an array and then work the array when we see a WriteQueuedBuffered back
            if (debug) console.log("inside sendBufferedJson. msg:", msg);
        ...
        sendBufferedDoNextJson: function() {
            ...
            if (debug) console.log("sendBufferedDoNextJson. sendbufjson:", this.sendbufjson);
            ...
            if (this.isSendBufWaitingJson) {
                if (debug) console.log("isSendBufWaitingJson is true, so returning.");
                //console.groupEnd();
                return;
            }
            ...
            if (this.isSingleSelectMode) {
                if (this.singleSelectPort != null && this.singleSelectPort.length > 1) {
                    ...
                    if (debug) console.log("setting isSendBufWaitingJson to true. sending to port:", this.singleSelectPort);
        ...
        sendViaJson: function(json) {
            ...
            if (debug) console.log("sendViaJson. json:", json);
            ...
            if ("D" in json) {
                ...
                if (debug) console.error("Sent incorrectly formatted object. You must provide the D parameter in your object to send commands to the serial port via JSON.");
        ...
        onQueuedText: function(data) {
            // we got a queued msg. good. fire off event
            if (debug) console.group("serial port widget - onQueuedText");
            ...
            if (this.versionFloat >= 1.7) {
                ...
                if (debug) console.log("not running 1.7 or later of SPJS. your version:", this.versionFloat);
            }
        ...
        onWriteJson: function(data) {
            ...
            if (debug) console.log("onWriteJson. data:", data);
        ...
        onCompleteJson: function(data) {
            ...
            if (debug) console.log("onCompleteJson. data:", data);
        ...
        onAnnounce: function(data){
            ...
            */
            ...
            if ('JsonTag' in data && data.JsonTag.length > 0) {
                if (debug) console.log("About to parse JsonTag:", data.JsonTag, " len of JsonTag:", data.JsonTag.length);
        ...
        sendNoBuf: function(msg) {
            ...
            $.each(listOfPortsToSendTo, function(i, port) {
                if (debug) console.log("sending to port:", port, "msg:", msg);
                msg = "send " + port + " " + msg;
                that.wsSend(msg);
            });
        ...
        wsConnect: function (hostname, onsuccess, onfail) {
            ...
            if (debug) console.log("websocket connect. hostname:", hostname);
            ...
            if (window["WebSocket"]) {
                ...
                if (!host) {
                    // see if cookie
                    if ($.cookie('lasthost')) {
                        if (debug) console.log("there is a previous hostname. use it.", $.cookie('lasthost'));
                ...
                if (location.protocol === 'https:') {
                    // page is secure
                    if (debug) console.log("this page requires ssl");
                    
                    if (fullurl.match(/wss/i)) {
                        // we're good to try cuz they want wss
                        if (debug) console.log("good, user wants wss. host:", fullurl);
                    } else {
                        if (debug) console.log("the websocket host url is not wss. host:", fullurl);
                ...
                if (debug) console.log(this.conn);
                var that = this;
        ...
        wsScan: function (callback, subnet) {
            ...
            // for hosts
            if (window["WebSocket"]) {
                if (debug) console.log("starting scan of network for serial port servers...");
                var validAddrs = [];
                var that = this;
                if (subnet)
                ...
                //for (var ctr = 1; ctr <= 26; ctr++) {
                  ...
                  setTimeout(function() {
                    ...
                    if (debug) console.log("attempting to connect to conn url:", conn.url);
                    conn.onopen = function (evt) {
                      scancntsuccess++;
                      if (debug) console.log("found a server. ip:", evt.target.url, "evt:", evt, "this:", this);
                      validAddrs.push(evt.target);
                      if (debug) console.log("found one:", validAddrs);
                    ...
                    conn.onmessage = function (evt) {
                      ...
                      if (debug) console.log("msg from scan of ws:", evt.data, "evt:", evt, "conn:", conn, "conn.url:", conn.url);
                      if (evt.data.match(/^{/)) {
                        // it's json
        ...
        serverListSet: function(key, val) {
          ...
          if (debug) console.log("serverList:", serverList, "txt:", txt);
          },
        serverListGet: function() {
          var serverList = localStorage.getItem("com-chilipeppr-widget-serialport-serverList");
          if (debug) console.log("got localStorage item:", serverList);
          if (serverList) {
          	serverList = JSON.parse(serverList);
            if (!serverList || serverList == null || serverList == "null") serverList = {};
            if (debug) console.log("after parse:", serverList);
            
          } else {
          	if (debug) console.log("got null so setting empty obj");
          	serverList = {};
          }
          return serverList;
        },
        ...
        publishSysMsg: function (msg) {
            ...
            if (this.lastMsg == msg && now - this.lastMsgTime < 20000) {
                // skip publish
                if (debug) console.log("skipping publish. same msg or too fast.");
            } else {
        ...
        publishMsg: function (msg) {
            if (this.isInDeDupeMode) {
                if (debug) console.log("in dedupe mode, so checking. msg:", msg, "lastMsg:", this.deDupeLastMsg);
                if (this.deDupeLastMsg == msg) {
                    // found dedupe, so don't publish
                    if (debug) console.log("dedupe");
            ...
            if (msg.match(/^\{/)) {
                ...
                */
                if (data && data.Cmd && data.Cmd == "OpenFail") {
                    ...
                    if (debug) console.log("got serial port list");
                    ...
                    if (this.isSingleSelectMode && this.singleSelectPort == data.P) {
                        ...
                        if (debug) console.log("have a /recvline to publish, but since in single select mode we don't have a match to the selected port so ignoring.");
                    }
        ...
        onPortOpen: function(data) {
            if (debug) console.group("onPortOpen");
            if (debug) console.log("Open a port: ", data, data.Port);
            var portname = data.Port;
            // swap back weird characters
            portname = this.toSafePortName(portname);
            if (debug) console.log("got onPortOpen. portname to use for dom lookups:", portname);
            $('#' + portname + "Cb").prop('checked', true);
            $('#' + portname + "Row .glyphicon-exclamation-sign").addClass("hidden");
            if (this.isSingleSelectMode) {
                if (debug) console.log("got port open but in single select mode.");
                // hilite this port
            ...
            if (debug) console.log("seeing if we should run a startup script. data:", data);
            ...
            if (debug) console.log("the script key:", key, "val is:", script);
        ...
        onPortClose: function(data) {
            if (debug) console.log("Close a port: ", data, data.Port);
            var portname = data.Port;
            portname = this.toSafePortName(portname);
        ...
        onPortOpenFail: function(data) {
            if (debug) console.log("Opening a port failed: ", data, data.Port);
            var portname = data.Port;
            portname = this.toSafePortName(portname);
            $('#' + portname + "Cb").prop('checked', false);
        ...
        onUpdateQueueCnt: function(data) {
            ...
            if (debug) console.log("got onUpdateQueueCnt. data:", data);
            var port = data.Port;
        ...
        setPortItemsFromMetaData: function(dm /*device meta*/, item /*port*/, portlistIndex) {
            ...
            if (debug) console.log("setting port data from device meta. port:", item, "device meta:", dm);
            ...
            if ('removeRelatedNames' in dm && dm.removeRelatedNames && 'RelatedNames' in item && item.RelatedNames) {
                ...
                if (debug) console.log("yes, they want us to remove related ports");
                ...
                for (var i = 0; i < item.RelatedNames.length; i++) {
                    ...
                    for (var portindex in this.portlist) {
                        ...
                        if (port.Name == portToDelete) {
                            if (debug) console.log("found port to delete. port:", port.Name);
                            port.isDeleted = true;
                            break;
                        }
                    }
            ...
            if (debug) console.log("at finish of setting port info from meta data. item:", this.portlist[portlistIndex]);
        },
        ...
        onPortList: function (portlist) {
            if (debug) console.group("serial port widget onPortList");
            ...
            if (debug) console.log("arduinoIcon:", arduinoIcon); //, "html:", arduinoIcon.parent.html());
            var found = false;
            if (portlist.length > 0) {
                $.each(portlist, function (portlistIndex, item) {
                    if (debug) console.log("looping thru ports. item:", item);
                    
                    // see if this is deleted
                    if (item.isDeleted) {
                        if (debug) console.log("this port is deleted so skipping");
                        return;
                    }
                    ...
                    if (debug) console.log("the port name we will use is:", i);
                    ...
                    if ('AvailableBufferAlgorithms' in item) {
                        ...
                        item.AvailableBufferAlgorithms.forEach(function(alg) {
                            //if (debug) console.log("algorithm:", alg);
                    ...
                    if (debug) console.log("looking for vid/pid or regexp:", item.UsbVid, item.UsbPid, item.regexp);
                    ...
                    for (var key in that.deviceMeta) {
                        ...
                        for (var indx = 0; indx < dm.vidpids.length; indx++) {
                            ...
                            if ('UsbVid' in item && cvp.vid.length > 0 && cvp.pid.length > 0 && item.UsbVid.toLowerCase() == cvp.vid.toLowerCase() && item.UsbPid.toLowerCase() == cvp.pid.toLowerCase() ) {
                                ...
                                if (debug) console.log("we have a match on vid/pid");
                        ...
                        if (isMatchOnVidPid) {
                            if (debug) console.log("since found vid/pid, break the for loop that is looping on meta data, cuz we're done");
                            break;
                        }
                        ...
                        if ('regexp' in dm && dm.regexp) {
                            ...
                            if (debug) console.log("looking for regexp:", dm.regexp);
                            if (item.Friendly.match(dm.regexp)) {
                                // we got a match, awesome
                                if (debug) console.log("we got a match on regexp on friendly");
                    ...
                    if (foundMeta) {
                        ...
                        if (debug) console.log("we had a match on meta data for this port, so known device");
                    } else {
                        // we did not have meta data, that means an unknown,
                        // so we have to show buffer encouragement
                        if (debug) console.log("we did not have meta data for this port.");
                    ...
                    if (debug) console.error("item friendly: " + String(item.Friendly));
                    if (String(item.Friendly).indexOf("K-PROX") !== -1)
                    {
                        found = true;
                        if (debug) console.log("found serial");
            ...
            if (!found) html = '<div class="alert alert-danger" style="margin-bottom:0;">No device connected.</div>';
            html = htmlFirst + html;
            $('.table.com-chilipeppr-widget-serialport-portlist tbody').html(html);
            
            if (debug) console.log("about to do baud/buffer override for isopen, cookie, meta, etc");
            $.each(portlist, function (i, item) {
                ...
                if (item.IsOpen) {
                    ...
                    if (that.versionFloat >= 1.4) {
                        if (debug) console.log("adding buffer bar");
                ...
                if (item.IsOpen == false && cookie && !('buffer' in cookie) && that.buffertype != null) {
                    if (debug) console.log("setting default buffer type cuz item not open, no cookie, and is a default buf type. item:", item);
                    $('#' + i + "Buffer").val(that.buffertype);
                }
            ...
            if (that.isInitting) {
                if (debug) console.log("setting isInitting to false so don't run init code again on port list");
                that.isInitting = false;
            }
            
            // now do last check on whether to hide buffer encouragement
            if (debug) console.log("about to check if we need to hide buffer encouragement. areAllDevicesKnown:", areAllDevicesKnown);
            if (areAllDevicesKnown == true) {
                // we can hide buffer encouragement
                if (debug) console.log("cuz all devices are known, we can safely hide");
                this.hideBufferEncouragement();
            }
        ...
        onPortSetDefaultClicked: function(evt) {
            if (debug) console.log("set default btn clicked for evt.data:", evt.data, "this.conn:", this.conn);
        ...
        onPortProgramClicked: function(evt) {
            if (debug) console.log("programmer btn got clicked for evt.data:", evt.data, "this.conn:", this.conn);
        ...
        onPortConfigClicked: function(evt) {
            if (debug) console.log("config got clicked for evt.data:", evt.data, "this.conn:", this.conn);
            this.hideAllPopovers();
            var port = evt.data.port.Friendly;
            var portid = evt.data.port.Name;
            var url = this.conn.url;
        ...
        onPortConfigModalSave: function(evt) {
            if (debug) console.log("got save on port config modal. evt.data:", evt.data);
            ...
            if (val.match(/\n$/)) {
                if (debug) console.log("there was newline at end. good.");
            } else {
                val += "\n";
                if (debug) console.log("there was no newline, so added one.");
            }
            if (debug) console.log("saving config for port key:", key, "val:", val, {key:key, val:val});
            ...
            if (debug) console.log("about to save to chilipeppr cloud. cpkey:", cpkey, "cpval:", cpval, "cpvalstr:", cpvalstr);
            var url = "http://www.chilipeppr.com/dataput?key=" + cpkey + "&val=" + cpvalstr + "&callback=?";
            // url = encodeURI(url);
            if (debug) console.log("len of url:", url.length, "url:", url);
            var jqxhr = $.ajax({
              url: url,
              dataType: 'jsonp',
        ...
        checkLogin: function() {
          ...
          var jqxhr = $.ajax({
              ...
              if (data.CurrentUser != undefined && data.CurrentUser != null) {
                if (debug) console.log("user logged in ", data.CurrentUser);
                ...
                if (debug) console.log("user NOT logged in");
            ...
            .fail(function() {
              if (debug) console.warn("Failed to make the ajax call to check ChiliPeppr login status");
            })
        },
        getUserDataKeysFromChiliPepprStorage: function() {

            if (debug) console.log("Doing getUserDataKeysFromChiliPepprStorage");
            ...
            .done(function(data) {
                ...
                data.forEach(function(item) {
                    ...
                    if (item.Name && item.Name.match(/com-chilipeppr-serialport-script-/)) {
                        ...
                        .done(function(data) {
                            if (debug) console.log("got data back from dataget call. data:", data);
                            if ("Value" in data) {
        ...
        onPortCbClicked: function(evt) {
            if (debug) console.log("got onPortClicked: ", evt);
            var item = evt.data.port;
            var that = evt.data.that;
            ...
            if (isSelected) {
               ...
               // var baud = $('#' + i + "Baud").val();
                var baud = 115200;
                if (debug) console.error("BAUD: " + baud);
        ...
        getBufferAlgorithms: function() {
            if (this.bufferAlgorithms == null) {
                if (debug) console.log("Need to load algorithms from serial port json server");
